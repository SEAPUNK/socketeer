...
class Client extends ClientAbstract {
  constructor (address, options = {}) {
    ...
    this.handshakeTimeout = options.handshakeTimeout || this.heartbeatTimeout
  ...
  _handleMessage (data, flags) {
    ...
    if (!this.ready) {
      switch (this._handshakeStage) {
        ...
        default:
          let err = new Error('[internal] unknown handshake stage: ' + this._handshakeStage)
  ...
  _handleStage1 (data, flags) {
    ...
    if (typeof data !== 'string' || data.indexOf('hi') !== 0) {
      this._d('stage 1 unexpected data, killing connection')
      return this.kill()
    }
    this._d('message is heartbeat interval')
    data = data.replace('hi', '')
    let interval = Math.floor(+data)
    if (Number.isNaN(interval) || interval < 1 || interval > 2147483647) {
      this._handleError(new Error('invalid heartbeat interval from server'))
      return
    }
  ...
  _resetHeartbeatTimeout () {
    ...
    this._heartbeatTimer = setTimeout(() => {
      ...
      //  point to doing a formal close, since that is also likely to
      //  time out.
      this.kill()
    }, timeoutPeriod)
  }
  ...
  _handleStage2 (data, flags) {
    ...
    if (this._resumeToken) {
      if (typeof data === 'string' && data.indexOf('r+') === 0) {
        let token = data.replace('r+', '')
        if (token.length < 5 || token.length > 200) {
          this._d('resume token length invalid')
          this._handleError(new Error('resume token length invalid'))
          return
        } else {
          ...
          this.emit('open', this.openState.SESSION_RESUME)
          return
        }
      } else if (data === 'r-') {
        ...
        this._resumeMessageQueue()
        this.emit('open', this.openState.SESSION_RESUME_FAIL)
        return
      } else {
        this._d('got unexpected data')
        this._handleError(new Error('got unexpected session resume data'))
        return
      }
    } else {
      if (data === 'r') {
        ...
        this._resumeToken = null
        this.ready = true
        let state = this.isReconnection ? this.openState.RECONNECTION : this.openState.NEW_CONNECTION
        this._clearMessageQueue()
        this._resumeMessageQueue()
        ...
        this._d('got unexpected data')
        this._handleError(new Error('got unexpected non-resume data'))
        return
      }
    }
  }
  ...
  reconnect (immediate) {
    ...
    // Clear out the resume token because we are not
    // going to resume the session.
    this._resumeToken = null
