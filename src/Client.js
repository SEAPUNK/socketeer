...
function Client (address, options) {
  ...
  this._socketeerEvents = {}  // This is because EventEmitter has
...
Client.prototype._handleEvent = function _handleEvent (data) {
  ...
  if (!this._socketeerEvents[data.e]) return
  var handlers = this._socketeerEvents[data.e]
...
Client.prototype.event = function event (name, handler) {
  ...
  if (!this._socketeerEvents[name]) this._socketeerEvents[name] = []
  this._socketeerEvents[name].push(handler)
}
