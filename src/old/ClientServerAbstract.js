...
const {ActionResponse} = require('./enums')
...
class ClientAbstract extends EventEmitter {
  constructor () {
    ...
    this._emitActionAsync = promisify(this._emitAction.bind(this))
    // Reserved variable for anyone to use.
    // Helps with not polluting the Socketeer namespace,
    // and accidentally overwriting required variables.
    this.data = {}
  }
  ...
  _processQueue (msg, done) {
    ...
    this._d(`[super] sending message: ${msg}`)
    this.ws.send(msg)
    return done()
  }
  ...
  send (obj) {
    ...
    this._d(`[super] adding data to send queue: ${data}`)
  ...
  _handleMessage (data, flags) {
    ...
    if (!this.isOpen()) {
      ...
      this._d('ignoring message, as socket is not open (this should never happen on ClientAbstract!)')
      return
    }
  ...
  request (name, data, callback) {
    if (typeof callback === 'function') {
      ...
      return this._emitActionAsync(name, data)
    }
  }
