...
# Don't use this version yet, it's still a WIP.

# TODO: globbing of handshake, to reduce the time it takes to handshake.
...
- [handshake finalization](#handshake-finalization)
...
- If the server *does* support session resuming, then it must generate a unique alphanumeric token (preferably long, and cryptographically secure) with a minimum length of 5 and a maximum length of 200 so both the server and client can store it for future use, and respond with `r:{TOKEN}`, where `{TOKEN}` is the generated token.
...
After completing all of the steps of the handshake, the client must send the "start" message: a single `s`. The server will then respond with `s` to allow the connection, and mark the connection as "ready", or otherwise close the connection.

```
implementation note

This is because of Socketeer's API. The API allows for 'middleware',
that will perform various actions before the connection is allowed,
e.g. custom authentication. If one of the middlewares fail (or reject the
connection), the connection is closed, and error is emitted.
```
