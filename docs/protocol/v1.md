...
Where `{INT}` is a positive integer (max 2147483647) that indicates how long in milliseconds the interval between each heartbeat will be.
...
If the server support session resuming, and the session exists, then server must devalidate the old token, respond with `r+{TOKEN}` (where `{TOKEN}` is a freshly generated token), and send any pending messages. Else, the server must respond with `r-`, and the client proceeds as if it was a new connection.
